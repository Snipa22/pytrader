...
from history.models import PredictionTest, Price, Trade, Balance, TradeRecommendation, get_time, PerformanceComp, ClassifierTest
...
def get_trade_chart(bs,denom,symbol,start_time):
    ...
    if settings.MAKE_TRADES:
        trades = Trade.objects.exclude(created_on_str="").filter(symbol=symbol,created_on__gte=start_time).filter(status__in=['fill','open','error']).order_by('id')
    else:
        trades = Trade.objects.exclude(created_on_str="").filter(symbol=symbol,created_on__gte=start_time).order_by('id')
...
def get_performance_comps_chart(bs,denom,symbol,start_time):
    ...
    ds = DataPool(
           series=
            [{'options': {
                'source': pcs},
              'terms': ['created_on_str','delta', 'actual_movement', 'nn_rec','pct_buy','pct_sell','weighted_avg_nn_rec' ]}
             ])
...
def profit_view(request):
    ...
    for t in Trade.objects.filter(symbol=symbol,status='fill').order_by('-created_on').all():
        ...
        data[date]['bal'] = Balance.objects.filter(symbol=this_symbol[1],created_on__lte=(datetime.datetime.strptime(date,'%Y-%m-%d')+datetime.timedelta(days= 1))).order_by('-created_on').values_list('coin_balance',flat=True).first()
    ...
    for key in data.keys():
        ...
        data[key]['unrealizeddiff'] = round(data[key]['bal'] * Price.objects.filter(symbol=symbol).order_by('-created_on').values_list('price',flat=True).first() * get_cost_basis(data[key]['bal'],symbol),4)
        ...
        view_data.append({
          'one_symbol' : '' if not 'symbol' in data[key].keys() else data[key]['symbol'],
          'unrealizeddiff' : '' if not 'unrealizeddiff' in data[key].keys() else data[key]['unrealizeddiff'],
          'date' : key,
          'symbol' : symbol,
